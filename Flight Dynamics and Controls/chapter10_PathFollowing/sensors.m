...
%    wd      = uu(24);
    ...
    % simulate accelerometers (units of m/s^2)
    y_accel_x = sat(F_x/P.mass + P.gravity*sin(theta)          + P.sigma_accel*randn(), P.accel_max, -P.accel_max);
    y_accel_y = sat(F_y/P.mass - P.gravity*sin(phi)*cos(theta) + P.sigma_accel*randn(), P.accel_max, -P.accel_max);
    ...
    y_diff_pres   = (P.rho*(Va^2)/2) + P.beta_diff + P.sigma_diff*randn();                           % kPa
