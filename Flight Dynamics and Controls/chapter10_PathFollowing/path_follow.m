...
function out = path_follow(in,P)
  ...
  switch flag,
      case 1, % follow straight line path specified by r and q
          ...
          % gather some information about the projection of e_p
          p_path = [pn; pe; -h];
          
          e_p = p_path - r_path;
          k_dir = [0, 0, 1];
          
          n = cross(q_path, k_dir)/norm(cross(q_path, k_dir));
          n = n';
          % this is the projection e_p denoted as s
          s = e_p - dot(e_p, n)*n;
          
          % calculate commanded height using equation 10.5
          h_c = -r_path(3) + sqrt(s(1)^2 + s(2)^2)*(-q_path(3)/(sqrt(q_path(1)^2 + q_path(2)^2)));
          
          % make sure that chi_q - chi is between -pi and pi
          ...
          % calculate commanded heading using equation 10.8
      ...
      case 2, % follow orbit specified by c, rho, lam
          ...
          % calculate barphi and make sure that we are in the bounds we
          % want
          ...
          chi_c = barphi + lam_orbit*(pi/2 + atan(P.k_orbit*((d-rho_orbit)/rho_orbit)));
          phi_ff = 0;
  end
