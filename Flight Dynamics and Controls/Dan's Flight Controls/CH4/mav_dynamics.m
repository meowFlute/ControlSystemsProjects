...
function sys=mdlDerivatives(t,x,uu, P)
    ...
    %Calculate Velocity (Vehicle Frame)
    pdotMatrix = [...
           cos(theta)*cos(psi) sin(phi)*sin(theta)*cos(psi)-cos(phi)*sin(psi) cos(phi)*sin(theta)*cos(psi)+sin(phi)*sin(psi); ...
           cos(theta)*sin(psi) sin(phi)*sin(theta)*cos(psi)+cos(phi)*cos(psi) cos(phi)*sin(theta)*cos(psi)-sin(phi)*cos(psi); ...
           -sin(theta) sin(phi)*cos(theta) cos(phi)*cos(theta) ...
           ];
    pdot = pdotMatrix * [u;v;w];
    pndot = pdot(1);
    pedot = pdot(2);
    pddot = pdot(3);

    %Calculate Accelerations (Body Frame)
