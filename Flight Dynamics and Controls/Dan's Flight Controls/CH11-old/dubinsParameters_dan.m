...
function dubinspath = dubinsParameters(start_node, end_node, R)
  ...
  if ell<2*R,
    ...
    % compute L1
    theta = angle(cre-crs);%atan2(norm(cross(crs,cre)),dot(crs,cre));
    L1 = norm(crs-cre)+R*mod2pi(2*pi+mod2pi(theta-pi/2)-mod2pi(chis-pi/2))...
            +R*mod2pi(2*pi+mod2pi(chie-pi/2)-mod2pi(theta-pi/2));
    % compute L2
    ell = norm(cle-crs);
    theta = angle(cle-crs);%atan2(norm(cross(crs,cle)),dot(crs,cle));
    theta2 = theta-pi/2*asin(2*R/ell);
    if isreal(theta2)==0, 
      L2 = 9999; 
    else
      L2 = sqrt(ell^2-4*R^2)+R*mod2pi(2*pi+mod2pi(theta2)-mod2pi(chis-pi/2))...
            +R*mod2pi(2*pi+mod2pi(theta2+pi)-mod2pi(chie+pi/2));
    end
    % compute L3
    ell = norm(cre-cls);
    theta = angle(cre-cls);%atan2(norm(cross(cls,cre)),dot(cls,cre));
    theta2 = acos(2*R/ell);
    if isreal(theta2)==0,
      L3 = 9999;
    else
      L3 = sqrt(ell^2-4*R^2)+R*mod2pi(2*pi+mod2pi(chis+pi/2)-mod2pi(theta+theta2))...
            +R*mod2pi(2*pi+mod2pi(chie-pi/2)-mod2pi(theta+theta2-pi));
    end
    % compute L4
    theta = angle(cle-cls);%atan2(norm(cross(cls,cle)),dot(cls,cle));
    L4 = norm(cls-cle)+R*mod2pi(2*pi+mod2pi(chis+pi/2)-mod2pi(theta+pi/2))...
